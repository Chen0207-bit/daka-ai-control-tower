...
const ledgerRows = [
  {id:"NU-2026-LIC",date:"2026.09.10",ip:"纽卡斯尔联",project:"年度授权费 · MG 下半年度分期",amount:"¥1,220,000",status:"待审批",owner:"王雯",basis:"授权合同第 4.2 条 · MG 半年付",note:"全年 MG ¥244 万分两期；净销售额超出 MG 部分按 12% 季度分成单列对账；Demo 推演"},
  ...
  {id:"ACM-SIG-03",date:"2026.10.15",ip:"AC 米兰",project:"人物资源第三批 · 签名包干",amount:"¥460,000",status:"待付款",owner:"林乔",basis:"人物资源补充协议",note:"按批包干，与第三批签名验收联动，超出份数按协议单价另计；Demo 推演"},
  {id:"ACM-ROY-Q3",date:"2026.11.15",ip:"AC 米兰",project:"Q3 授权分成 · 超保底溢出结算",amount:"¥186,000",status:"待对账",owner:"王雯",basis:"授权合同分成条款 · 超出 MG 部分 10%",note:"按 Q3 净销售额推演口径计算，需与版权方对账单核对后支付；Demo 推演"},
  {id:"PB-TALENT-01",date:"2026.11.30",ip:"浴血黑帮",project:"首批演员资源 · 立项预算",amount:"¥780,000",status:"预算中",owner:"周宁",basis:"立项预算草案",note:"授权拟按 MG+分成结构签约，首年 MG 尚未锁定；当前为预算占用，未形成正式义务；Demo 推演"},
  {id:"ACM-LIC-H2",date:"2026.12.15",ip:"AC 米兰",project:"年度授权费 · MG 下半年度分期",amount:"¥600,000",status:"已排期",owner:"王雯",basis:"授权合同 · MG 半年付",note:"全年 MG ¥120 万分两期；Demo 推演"},
];
...
function Portfolio({ ips, onImport, onOpenIP }: { ips: typeof baseIps; onImport: () => void; onOpenIP: (code:string) => void }) {
  ...
  <section className="panel license-panel">
    ...
    <div className="mg-grid">{mgAlerts.map(card=><div className={`mg-card ${card.level==="高"?"hot":""}`} key={card.ip}><div className="mg-head"><span className={`mg-level level-${card.level}`}>{card.level}</span><b>{card.ip}</b></div><h3>{card.title}</h3><p>{card.detail}</p><span className="mg-action">建议动作 · {card.action}</span></div>)}</div>
  </section></>;
}
...
function Blueprint() {
  ...
  return <><PageHeader eyebrow="EXPANSION BLUEPRINT" title="敲门砖之后，是一套可生长的经营系统" sub="第一阶段先让老板掌控 IP；后续模块沿统一的人物、卡牌、项目与账目数据继续扩展。" />
