...
const signals = [
  { date: "2026.08.25", level: "高", type: "转会官宣", person: "Christopher Nkunku", ip: "AC 米兰", headline: "AC 米兰发布 Christopher Nkunku 官方公告", source: "AC Milan 官方", url: "https://www.acmilan.com/en/transfer-window", impact: "如人物处于现有或候选卡表，应立即复核肖像授权、签名排期和卡面终审版本。", status: "待研判", matched: true, release: true },
  { date: "2026.08.27", level: "高", type: "新援官宣", person: "Nico González", ip: "纽卡斯尔联", headline: "纽卡斯尔联官宣签下 Nico González", source: "Newcastle United 官方", url: "https://www.newcastleunited.com/en/news/newcastle-united-complete-nico-gonzalez-signing", impact: "候选人物池新增高关注中场；建议评估是否进入下一季基础卡、签名卡与营销物料。", status: "已匹配", matched: true, release: true },
  { date: "2026.08.14", level: "中", type: "外借官宣", person: "Zachary Athekame", ip: "AC 米兰", headline: "Zachary Athekame 租借加盟里昂", source: "AC Milan 官方", url: "https://www.acmilan.com/en/news/articles/media/2026-08-14/official-statement-zachary-athekame", impact: "若已列入在途卡表，需要核对球衣素材、俱乐部归属标注和发行区域审批。", status: "已确认", matched: true, release: false },
];

const metricTasks: Record<MetricKey, { title:string; owner:string; due:string; status:string; ai:string }[]> = {
  decision: [
    {title:"AC 米兰签名缺口：选择保期方案",owner:"林乔",due:"今天 18:00",status:"待老板决定",ai:"已核对台账并生成 3 套方案"},
    {title:"纽卡斯尔授权费：全额或分期",owner:"王雯",due:"09 月 08 日",status:"待老板决定",ai:"已计算 90 天现金影响"},
  ],
  delegated: [
    {title:"浴血黑帮核心演员代理询价",owner:"周宁",due:"今天",status:"执行中",ai:"已起草 6 封询价邮件"},
    {title:"AC 米兰第三批素材验收",owner:"林乔",due:"10 月 15 日",status:"已授权",ai:"持续追踪交付与验收人"},
    {title:"纽卡斯尔新援候选卡表复核",owner:"陈恺",due:"本周五",status:"已授权",ai:"已匹配官方新援公告"},
  ],
  external: [
...
const ledgerRows = [
  {id:"NU-2026-LIC",date:"2026.09.10",ip:"纽卡斯尔联",project:"年度授权费",amount:"¥1,220,000",status:"待审批",owner:"王雯",basis:"授权合同第 4.2 条",note:"需在付款前补齐验收确认；Demo 数据"},
  {id:"ACM-SIG-03",date:"2026.10.15",ip:"AC 米兰",project:"人物资源第三批",amount:"¥460,000",status:"待付款",owner:"林乔",basis:"人物资源补充协议",note:"与第三批签名验收联动；Demo 数据"},
  {id:"PB-TALENT-01",date:"2026.11.30",ip:"浴血黑帮",project:"首批演员资源",amount:"¥780,000",status:"预算中",owner:"周宁",basis:"立项预算草案",note:"尚未形成正式付款义务；Demo 数据"},
  {id:"SVC-AGENCY-Q3",date:"2026.09.25",ip:"共享服务",project:"海外代理协调服务费",amount:"¥86,000",status:"待核对",owner:"采购",basis:"季度服务单",note:"覆盖三个 IP 的代理沟通；Demo 数据"},
  {id:"SVC-LEGAL-09",date:"2026.10.05",ip:"共享服务",project:"授权合同法律复核",amount:"¥35,000",status:"已排期",owner:"法务",basis:"专项服务报价",note:"用于付款条款与地域授权复核；Demo 数据"},
];
...
export default function Home() {
  ...
  return <main className={`app-shell ${collapsed ? "is-collapsed" : ""}`}>
    ...
    <section className="workspace">
      <div className="utility-bar"><button className="mobile-menu" onClick={() => setCollapsed(v => !v)}>☰</button><div><span className="system-dot" />数据已同步 <b>2 分钟前</b></div><button className="quick-import" onClick={() => openImport("ip")}>＋ 导入数据</button><span className="avatar" aria-label="当前用户 FC">FC</span></div>
      ...
      {view === "portfolio" && <Portfolio ips={allIps} onImport={() => openImport("ip")} onOpenIP={(code) => {setReleaseProjectId(code==="NU"?"nufc":code==="PB"?"pb":"acm");setView("release");}} />}
...
function Portfolio({ ips, onImport, onOpenIP }: { ips: typeof baseIps; onImport: () => void; onOpenIP: (code:string) => void }) {
  ...
  return <><PageHeader eyebrow="IP PORTFOLIO" title="现有 IP 组合，一眼看到风险与下一步" sub="当前展示真实业务样例；数据结构可支撑 20 个 IP，不再展示空占位符。" actions={<button className="primary" onClick={onImport}>＋ 导入新 IP</button>} /><div className="filter-row"><button className={filter==="all"?"selected":""} onClick={()=>setFilter("all")}>全部 {ips.length}</button><button className={filter==="risk"?"selected":""} onClick={()=>setFilter("risk")}>有风险 2</button><button className={filter==="pay"?"selected":""} onClick={()=>setFilter("pay")}>90 天内付款 2</button><button className={filter==="start"?"selected":""} onClick={()=>setFilter("start")}>待启动 1</button><span>当前显示 {visible.length} · 容量 {ips.length} / 20</span></div><section className="portfolio-grid">{visible.map(ip => <button className="ip-card" key={`${ip.code}-${ip.name}`} onClick={()=>onOpenIP(ip.code)}><div className={`ip-mark ${ip.tone}`}>{ip.code}</div><div className="ip-card-head"><div><h2>{ip.name}</h2><p>{ip.kind} · {ip.contract}</p></div><span>{ip.stage}</span></div><div className="health"><div><small>资源健康度</small><b>{ip.health}%</b></div><i><em style={{width:`${Math.min(100, ip.health)}%`}} /></i></div><div className="ip-facts"><div><small>近期付款</small><b>{ip.pay}</b></div><div><small>当前提醒</small><b>{ip.risk}</b></div></div></button>)}</section></>;
}
